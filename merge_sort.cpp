...
#include <time.h>
    ...
    void MergeSort(int i_poczatek, int i_koniec) //"i_poczatek" czyli indeks pierwszego elementu zbioru sortowanego, "i_koniec" - indeks końcowego elementu zbioru
    {
        ...
        for(i_schowek = i_poczatek; i_schowek <= i_koniec; i_schowek++)//Porównujemy ze sobą najmniejsze elementy scalanych zbiorów. Ponieważ zbiory te są już uporządkowane, to najmniejszymi elementami będą zawsze ich pierwsze elementy.
        {
   ...
   /* int main()
    {
        ...
        {
            cout<< "wpisz " << j+1 <<" liczbe do posortowania i nacisnij enter"<<endl;
            cin>>liczba;
            tablica_glowna[j]=liczba;
        }
